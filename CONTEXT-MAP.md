# Context Map

This is a monorepo. Each package and app has its own context.

- [apps/web](./apps/web/CONTEXT.md)
- [apps/gateway](./apps/gateway/CONTEXT.md)
- [apps/identity-service](./apps/identity-service/CONTEXT.md)
- [apps/restaurant-service](./apps/restaurant-service/CONTEXT.md)
- [apps/food-service](./apps/food-service/CONTEXT.md)
- [apps/order-service](./apps/order-service/CONTEXT.md)
- [apps/inventory-service](./apps/inventory-service/CONTEXT.md)
- [apps/payment-service](./apps/payment-service/CONTEXT.md)
- [packages/ui](./packages/ui/CONTEXT.md)
- [packages/eslint-config](./packages/eslint-config/CONTEXT.md)
- [packages/typescript-config](./packages/typescript-config/CONTEXT.md)

## AI Agent Workflow

Before implementing any feature:

1. Read SYSTEM_CONTEXT.md
2. Read relevant arc42 architecture docs
3. Read related ADRs
4. Read package CONTEXT.md
5. Create implementation plan
6. Execute
7. Update documentation
8. Commit and push


## AI Agent Workflows

**Before frontend work:**
1. Read docs/frontend/frontend-quality-guidelines.md
2. Read docs/frontend/component-guidelines.md
3. Read related CONTEXT.md
4. Create UI implementation plan

**Before backend work:**
1. Read docs/backend/backend-architecture.md
2. Read ADRs
3. Read service CONTEXT.md
4. Create API implementation plan
