...
describe('Payment Service E2E Hardening Test', () => {
  ...
  it('SCENARIO 1: Duplicate OrderCancelled -> exactly ONE PaymentRefunded', async () => {
    ...
    pubChannel.publish('foodiego.orders.events', 'OrderCancelled', Buffer.from(JSON.stringify(orderCancelledPayload)));
    pubChannel.publish('foodiego.orders.events', 'OrderCancelled', Buffer.from(JSON.stringify(orderCancelledPayload)));
    pubChannel.publish('foodiego.orders.events', 'OrderCancelled', Buffer.from(JSON.stringify(orderCancelledPayload)));
    ...
    const outboxEvents = await pool.query(`SELECT * FROM outbox_events WHERE status = 'PENDING' AND aggregate_id = $1`, [paymentId]);
    for (const event of outboxEvents.rows) {
      ...
      pubChannel.publish('foodiego.payment.events', event.event_type, Buffer.from(JSON.stringify(envelope)));
      await pool.query(`UPDATE outbox_events SET status = 'PUBLISHED' WHERE event_id = $1`, [event.event_id]);
    }
    await setTimeout(500);

    const finalPayment = await pool.query(`SELECT status, is_refund_requested FROM payments WHERE id = $1`, [paymentId]);
    ...
    const allOutboxEvents = await pool.query(`SELECT * FROM outbox_events WHERE aggregate_id = $1`, [paymentId]);
  ...
  it('SCENARIO 2: Duplicate Webhook with older sequence -> exactly ONE PaymentAuthorized', async () => {
    ...
    await paymentRepo.persistWebhookInbox(crypto.randomUUID(), 'mock', 'tx_1', 'sig', 'hash', {
    ...
    const finalPayment = await pool.query(`SELECT status, gateway_sequence FROM payments WHERE id = $1`, [paymentId]);
