...
export function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await api.post<{ success: boolean; data: { token: string; user: any } }>('/auth/login', { email, password, role: 'admin' });
      if (res.data.success) {
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      <div className="w-full max-w-md p-8 space-y-4 border rounded-xl shadow-sm bg-card">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" className="w-full py-2 font-bold text-white bg-red-600 rounded hover:bg-red-700">Access System</button>
        </form>
      </div>
    </div>
  );
