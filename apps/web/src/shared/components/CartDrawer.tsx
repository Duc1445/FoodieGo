...
export function CartDrawer() {
  ...
  const { items, restaurant, summary, pendingItemIds, isLoadingCart, actions } = useCartStore();
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {isLoadingCart ? (
            <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-4">
              <ShoppingCart className="w-16 h-16 opacity-20 animate-pulse" />
              <p>Syncing cart...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-4">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4">
                {items.map((item) => {
                  ...
                  return (
                    <div key={item.id} className={`flex gap-4 p-3 border rounded-lg bg-card transition-opacity ${isPending ? 'opacity-60' : ''}`}>
                      <div
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                          ...
                          <button
                            ...
                            disabled={isPending}
                            className="text-muted-foreground hover:text-destructive transition-colors ml-2 disabled:opacity-50"
                          >
