...
describe('Menu Model', () => {
  ...
  it('findAll - should return rows and total', async () => {
    pool.query
      .mockResolvedValueOnce({ rows: [{ count: '1' }] })
      .mockResolvedValueOnce({ rows: [{ id: '1' }] });
    
    const res = await repo.findAll({ page: 1, limit: 10 });
  ...
  it('create - should return created row', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ id: '1' }] });
    const res = await repo.create({ name: 'Menu', price: 10, category_id: 'cat-1' });
    expect(res).toEqual({ id: '1' });
  });

  it('update - should return updated row', async () => {
    pool.query.mockResolvedValueOnce({ rows: [{ id: '1' }] });
    const res = await repo.update('1', { name: 'Menu2' });
    expect(res).toEqual({ id: '1' });
  });
