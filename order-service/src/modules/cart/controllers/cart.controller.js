...
export const addItem = async (req, res, next) => {
  try {
    ...
    const item = await cartService.addItem(req.user.id, menu_id, quantity);
...
export const updateItem = async (req, res, next) => {
  try {
    const item = await cartService.updateItem(req.user.id, req.params.menuId, req.body.quantity);
...
export const removeItem = async (req, res, next) => {
  try {
    const removed = await cartService.removeItem(req.user.id, req.params.menuId);
