...
export class OutboxDispatcher {
  ...
  constructor(pool, publisher, options = {}) {
    ...
    this.workerId = options.workerId || `dispatcher-${process.pid}-${Math.random().toString(36).substring(7)}`;
  ...
  async stop() {
    console.log(`[OutboxDispatcher] Stopping worker ${this.workerId}... waiting for current batch to finish.`);
    this.running = false;
  }
  ...
  async _processBatch() {
    ...
    try {
      ...
      await client.query('ROLLBACK');
      console.error('[OutboxDispatcher] Failed to update statuses, events will be reclaimed after lease expires');
    } finally {
      client.release();
    }
